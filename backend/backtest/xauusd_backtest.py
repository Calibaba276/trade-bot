"""
Standalone backtest harness for the XAUUSD (Gold) ICT strategy (backend/strategies/xauusd_model.py).

This is a faithful, dependency-light re-implementation of the decision logic in
XAUUSDModel.on_trading_iteration. It does NOT import lumibot / supabase / MT5 (none of
...
Data source: Oanda XAU/USD (Gold) 1-minute bars (2006-2020) from the GitHub-hosted
...
BUFFER = 0.50          # self.buffer in $ (Gold trades in USD/oz, not pips)
MIN_FVG_SIZE = 0.50    # discard FVGs narrower than this ($) — Gold's noisy wicks
    ...
    def step(self, i, dt, current_date, current_time):
        ...
        if time(9, 0) <= current_time < time(11, 0) and not s.traded_london:
            ...
            if s.mss_swing_low and last_price < s.mss_swing_low and not s.bearish_fvg_confirmed:
                ...
                if c1 > c2 and (c1 - c2) >= MIN_FVG_SIZE:
                    s.fvg_top = c1
                    s.fvg_bottom = c2
            ...
            if s.mss_swing_high and last_price > s.mss_swing_high and not s.bullish_fvg_confirmed:
                ...
                if c1 < c2 and (c2 - c1) >= MIN_FVG_SIZE:
                    s.fvg_top = c2
                    s.fvg_bottom = c1
                ...
                if s.ny_sweep_high and last_price < s.ny_range_high and not s.traded_ny:
                    ...
                    if s.mss_swing_high and last_price > s.mss_swing_high:
                        ...
                        if df is not None and not df.empty and len(df) >= 3:
                            c1 = float(df.iloc[-3]["high"])
                            c2 = float(df.iloc[-1]["low"])
                            if c2 > c1 and (c2 - c1) >= MIN_FVG_SIZE:
                                s.fvg_top = c2
                                s.fvg_bottom = c1
                                s.bullish_fvg_confirmed = True
                                s.bearish_fvg_confirmed = False
...
# ── Reporting ─────────────────────────────────────────────────────────────────
def report(signals, pair="XAU_USD"):
    total = len(signals)
    ...
    print("=" * 68)
    print(" XAUUSD (GOLD) ICT BACKTEST — {} 1m (Oanda) — fixed RR {:.1f}, ${:.0f}/trade".format(pair.replace("_", "/"), RR_RATIO, RISK))
    print("=" * 68)
    ...
    # Round-trip cost in $ = spread($/oz) * units. Gold spread quoted in dollars.
    print("\n Cost sensitivity (round-trip spread in $/oz applied to every filled trade):")
    for spread in (0.0, 0.10, 0.20, 0.30, 0.50):
        cost = 0.0
        for s in resolved:
            units = RISK / abs(s.entry - s.sl)
            cost += spread * units
        print(f"   spread ${spread:>4.2f}/oz -> net P&L ${net - cost:>10,.0f}   (costs ${cost:>9,.0f})")
    return curve
