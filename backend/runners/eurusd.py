...
def run():
    ...
    logger.info(
        f"=== EURUSD runner starting === mode={mode} account={account} server={server}"
    )

    if str(is_backtesting).lower() == "true":
        logger.info("EURUSD runner: entering BACKTEST branch")
        ...
        logger.info("EURUSD runner: entering LIVE branch")
        try:
            ...
            logger.exception(f"EURUSD runner crashed: {type(exc).__name__}: {exc}")
