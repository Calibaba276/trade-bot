...
| **Audit links** | Any session can be shared as a tamper-proof public link — recipients see the exact snapshot the owner exported, no account required |
...
│   ├── services/             # orchestrator, worker, position_monitor, verdict,
│   │                         #   publisher, frontend_emitter, telegram_notifier, vault
      ...
      └── build_verdict()
          ...
          └── publish_verdict() → Redis "signals" channel
                └── worker.py receives signal
                    ...
                    └── telegram_notifier.py → Telegram alert
```
...
| [APScheduler](https://apscheduler.readthedocs.io/) | Scheduled background tasks |
